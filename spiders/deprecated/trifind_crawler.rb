module Crawl
  class TrifindCrawler < Spidey::AbstractSpider
    include Spidey::Strategies::Mongo

    def self.db
      @@db ||= Mongo::Connection.new['trifind_crawler']
    end
    ...
    def process_racelist(page, data = {})  # {:state=>"Alaska Triathlons"}
      ...
      page.search("a.button[title='Next']").each do |a|
        ...
        handle resolve_url("/?page=#{next_num}", page), :process_racelink,
